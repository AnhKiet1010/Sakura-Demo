    ...
    return (
        <li className="block p-1 w-full" id={id}>
            <article tabIndex="0"
                ...
                <video controls className="w-full h-full sticky object-cover rounded-md bg-fixed">
                    <source src={data} type="video/mp4" />
                            Your browser does not support HTML video.</video>
                <section
                    className="flex flex-col hover:text-white rounded-md text-xs break-words w-full h-full z-20 absolute top-0 py-2 px-3">
                    <h1 className="flex-1">{filename}</h1>
                    <div className="flex">
                        <span className="p-1">
                            <i>
                                <svg className="fill-current w-4 h-4 ml-auto pt-"
